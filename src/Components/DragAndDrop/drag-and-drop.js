...
compoundDragAndDrop.prototype.run = function() {
    ...
    const reset = (grabbedNode, dropTarget, dropTargetBounds, boundsTuples) => {
        grabbedNode.removeClass('cdnd-grabbed-node');
        dropTarget.removeClass('cdnd-drop-target');
    
        grabbedNode = cy.collection();
        dropTarget = cy.collection();
    ...
    //     dropTarget.addClass('cdnd-drop-target');
    ...
    cy.on('free', 'node', (event) => {
        // Add the node here. 
        if (chosenParent.length > 0) {
            const position = getPositionCopy(event.target.position());
            if ((potentialMovingNode.data('element_type') === 'common*' || potentialMovingNode.data('element_type') === 'common') && potentialMovingNode.data('partition') === chosenParent.id()) {
